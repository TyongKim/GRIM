...
gfundata(1).expression = '3+(x(:,1)-x(:,2)).^2./10-(x(:,1)+x(:,2))/sqrt(2)';
gfundata(2).expression = '3+(x(:,1)-x(:,2)).^2./10+(x(:,1)+x(:,2))/sqrt(2)';
gfundata(3).expression = 'x(:,1)-x(:,2)+7/sqrt(2)';
gfundata(4).expression = 'x(:,2)-x(:,1)+7/sqrt(2)';
...
system_data.type    = 'series';      %(series/parallel/general/no)
